...
case "$OS_RELEASE" in 
  9*)
    ...
    BOMFILE_MACFUSE="$INSTALL_VOLUME/Library/Receipts/boms/com.github.osxfuse.pkg.MacFUSE.core.bom"
    ;;
  10*|11*|12*|13*|14*)
    ...
    BOMFILE_MACFUSE="$INSTALL_VOLUME/var/db/receipts/com.github.osxfuse.pkg.MacFUSE.bom"
    ;;
esac
...
kextunload -b com.github.osxfuse.filesystems.osxfuse > /dev/null 2>&1
...
then
  ...
  then
    "`dirname "$0"`/uninstall_macfuse.sh"
  fi
fi
...
remove_file "$INSTALL_VOLUME/./Library/Filesystems/osxfuse.fs/Contents/Resources/autoinstall-osxfuse-core"
